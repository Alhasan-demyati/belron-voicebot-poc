...
This is the **second** ElevenLabs Conversational AI agent that picks up calls when Carla escalates. For the POC its job is to demonstrate the handover by speaking with a clearly different (male) voice and continuing the same playbook as the primary Carla agent.

## When this agent receives a call

The primary agent (`Carla DE PoC`) calls the n8n `transfer_to_agent` webhook. n8n logs the handover and returns `target_agent_id` (this agent's ElevenLabs ID, set via the `CARGLASS_HANDOVER_AGENT_ID` n8n variable). The actual call transfer is performed via ElevenLabs' native **Agent Transfer** tool, which must be wired in the primary agent's tools panel — the n8n webhook is only the data-plane (logging + qualification + summary).

## Identity

| Field | Value |
|---|---|
| Agent name | `Carla DE — Handover (Max)` |
| Description | `Sekundärer Agent (männliche Stimme), übernimmt Übergaben vom primären Carla-Agent.` |
...
| Stability | `0.50` | Same as Carla |
| Similarity boost | `0.75` | Same as Carla |
| Style exaggeration | `0.00` | Same as Carla |
| Speaker boost | `true` | Same as Carla |
...
When this agent picks up after a transfer, it should **not** greet from scratch — it already inherits context from Carla. Use a continuation greeting:

```
Hier ist Max, ich übernehme von Carla. Ich habe Ihren bisherigen Hergang. Wie kann ich Ihnen weiterhelfen?
```

(English fallback if the conversation language was English:)

```
This is Max, taking over from Carla. I have your conversation context. How can I help you?
```

## System prompt

**Use the SAME system prompt as Carla** — see [system_prompt.md](system_prompt.md). Only differences:

1. Replace the identity line "You are **Carla**" with "You are **Max**".
2. Replace the greeting in Section 4 with the continuation greeting above (you are not the entry point; you do not re-ask consent — Carla already handled it).
...
In ElevenLabs' **primary** Carla agent, configure their built-in **Agent Transfer** capability (Conversational AI → Tools → Agent transfer). Point it at this agent's `agent_id`. Carla's `transfer_to_agent` n8n call is the **data-plane** (logs the handover, builds the summary). The **control-plane** transfer (the actual SIP/WebRTC call routing) happens via ElevenLabs's native agent-transfer tool, triggered immediately after the n8n call returns `ok: true`.
...
After saving, copy this agent's system prompt + tool schema + voice ID into Supabase `agent_versions` with `is_active = true` and a different `version_label` (e.g., `carla-de-max-v0.1`). The dashboard's version-comparison page will then surface both Carla and Max side by side.
